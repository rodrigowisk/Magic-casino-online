      ...
      <div class="modal-content">
        ...
        <div v-if="jogadorSelecionado" class="modal-player-mini">
          ...
          <div class="mini-info">
            ...
            <span class="mini-balance">Saldo atual: <strong>🪙 {{ formatarMoeda(jogadorSelecionado.saldo) }}</strong></span>
          </div>
        </div>

        <div class="modal-input-group">
          <label for="valor-transacao">Quantidade (🪙)</label>
...
const afiliados = ref([
  { id: '#678967696978', nome: 'rodrigowisk', saldo: 1500.50, avatarUrl: 'rodrigo_avatar_oficial' }, 
  { id: '#889900112233', nome: 'maria_cassino', saldo: 250.00, avatarUrl: 'maria_avatar_oficial' },  
  { id: '#445566778899', nome: 'jogador_vip10', saldo: 12400.75, avatarUrl: 'vip10_avatar_oficial' }, 
]);
...
const confirmarTransacao = () => {
  ...
  if (tipoModal.value === 'depositar') {
    console.log(`Depositando 🪙 ${valor} para ${jogadorSelecionado.value.nome}`);
    ...
    console.log(`Sacando 🪙 ${valor} de ${jogadorSelecionado.value.nome}`);
    jogadorSelecionado.value.saldo -= valor; 
  }
...
onMounted(async () => {
  const actualAvatar = localStorage.getItem('magic_avatar') || 'default.webp';
  
  afiliados.value = afiliados.value.map(jogador => {
    if (jogador.nome === 'rodrigowisk') {
      return { ...jogador, avatarUrl: actualAvatar };
    }
