...
export interface GameHubCallbacks {
    ...
    // 🔥 Disparado quando o SignalR reconecta sozinho (queda de conexão momentânea).
    // Diferente de uma reconexão manual/reload, o app Vue continua vivo — então
    // sem esse gancho, o estado local pode achar que já sabe tudo e nunca força
    // uma resincronização completa, deixando o board preso num estado antigo.
    onReconnected?: () => void;
}
    ...
    const connect = async () => {
        try {
            ...
            hubConnection.on("ReceiveError", (msg: string) => {
                // 👇 INTERCEPTADOR GLOBAL DE ERROS DO SERVIDOR (CUIDA DO TOKEN VENCIDO ENVIADO VIA MENSAGEM)
                if (msg.includes("401") || msg.toLowerCase().includes("unauthorized") || msg.toLowerCase().includes("expirado") || msg.toLowerCase().includes("token")) {
                    console.warn("Token expirado interceptado via evento do servidor. Redirecionando para login...");
            ...
            hubConnection.onclose(async (error) => {
                // 👇 INTERCEPTADOR GLOBAL DE QUEDA DE CONEXÃO (CUIDA DO TOKEN VENCIDO DURANTE O JOGO)
                if (error && (error.message.includes("401") || error.message.toLowerCase().includes("unauthorized"))) {
                     console.warn("Conexão fechada por token expirado (401). Redirecionando para login...");
                     localStorage.removeItem('magic_token');
                     window.location.href = '/login';
                     return;
                }
            ...
            // 👇 INTERCEPTADOR GLOBAL DE CONEXÃO INICIAL (CUIDA DO TOKEN VENCIDO AO TENTAR ENTRAR NA MESA)
            const errorMsg = err?.toString() || "";
            if (errorMsg.includes("401") || errorMsg.includes("Unauthorized") || errorMsg.includes("StatusCode: 401")) {
                console.warn("Falha de autenticação ao conectar (401). Redirecionando para login...");
                
                // Limpa o token estragado da memória para garantir que não vai ficar em loop
                localStorage.removeItem('magic_token');
