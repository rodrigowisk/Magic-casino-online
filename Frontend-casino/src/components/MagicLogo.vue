...
onMounted(async () => {
  if (pixiContainer.value) {
    ...
    // 👇 CORREÇÃO 1: 'blur' alterado para 'strength' (Sintaxe PixiJS v8)
    const cosmoBlur = new PIXI.BlurFilter();
    cosmoBlur.strength = 4;
...
onBeforeUnmount(() => {
  // 👇 CORREÇÃO 2: Descarrega a textura corretamente usando o sistema novo de Assets
  PIXI.Assets.unload(logoImg);

  if (app) {
    // 👇 Removido o 'texture: true' para evitar que destrua o que o Assets gerencia
