          ...
          <div class="players-list">
            <div v-for="(player, index) in currentRound.players" :key="index" class="history-item">
              ...
              <div class="info-section">
                <span class="player-name" :class="{ 'is-me': isMe(player.playerId) }">
                  {{ truncateName(getCorrectPlayerName(player)) }}
                </span>
              ...
              <div class="hole-cards">
                <template v-if="player.betAmount > 0 || isMe(player.playerId)">
...
// Helper para descobrir de forma segura se o jogador é o próprio usuário atual
const isMe = (playerId: string) => {
  if (!playerId || !props.currentUserId) return false;
  return String(playerId).toLowerCase() === String(props.currentUserId).toLowerCase();
};

// 👇 LÓGICA ULTRA-ROBUSTA E COMPATÍVEL COM CASING DO .NET 👇
const getCorrectPlayerName = (historyPlayer: any) => {
  // 1. Prioridade Máxima: O nome real salvo no banco de dados!
  if (historyPlayer.playerName && historyPlayer.playerName !== 'Jogador') {
    return historyPlayer.playerName;
  }

  if (!historyPlayer || !historyPlayer.playerId) return 'Jogador';

  const searchId = String(historyPlayer.playerId).toLowerCase();

  // 1. Identifica os Bots imediatamente ignorando a caixa (maiuscula/minuscula)
  ...
  if (botNames[searchId]) {
    return botNames[searchId];
  }

  // 2. Busca o jogador se ele ainda estiver na mesa (Mapeamento Resiliente)
  if (props.players) {
    // Garante compatibilidade caso props.players venha como Array ou Object/Dicionário
    const playersArray = Array.isArray(props.players) ? props.players : Object.values(props.players);

    const livePlayer = playersArray.find((p: any) => {
      if (!p) return false; // Evita quebras se o assento estiver vazio (null)

      // Suporta estrutura plana ou aninhada (ex: se o jogador vier encapsulado em p.player)
      const target = p.player ? p.player : p;
      if (!target) return false;

      // Resiliência total contra variações de propriedades do .NET/SignalR (userId, UserId, playerId, etc.)
      const pId = target.userId || target.UserId || 
                  target.playerId || target.PlayerId || 
                  target.id || target.Id || '';

      return String(pId).toLowerCase() === searchId;
    });

    if (livePlayer) {
      const target = livePlayer.player ? livePlayer.player : livePlayer;
      const name = target.name || target.Name;
      if (name) return name; 
    }
  }

  // 3. Verifica se é você mesmo
  if (isMe(historyPlayer.playerId)) {
    return localStorage.getItem('magic_username') || 'Você';
  }

  // 4. Retorno de segurança padrão
  return 'Jogador';
};
