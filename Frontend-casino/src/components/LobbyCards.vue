...
const props = defineProps<{
    room: any;
    // 👇 CORRIGIDO AQUI DE number PARA string
...
// 🔥 NOVO: Verifica se está no modo Treino para trocar o ícone da ficha 🔥
const isDemoMode = ref(localStorage.getItem('magic_lobby_mode') !== 'VIP');

const updateMode = () => {
    isDemoMode.value = localStorage.getItem('magic_lobby_mode') !== 'VIP';
};

onMounted(() => {
    window.addEventListener('lobby-mode-changed', updateMode);
});

onUnmounted(() => {
    window.removeEventListener('lobby-mode-changed', updateMode);
});
...
// 🔥 MAQUIAGEM: Casas decimais removidas para economizar espaço e focar no "Social"
    ...
    <div style="width: 180px; height: 265px; border-radius: 12px; cursor: pointer; position: relative; background-color: #151515; border: 1px solid rgba(255,255,255,0.05);" class="shadow-lg hover:scale-[1.03] transition-transform duration-300 group" @click="handleAction">
        ...
        <div style="position: absolute; top: calc(57% + 20px); bottom: 44px; left: 0; right: 0; z-index: 30; display: flex; justify-content: center; align-items: center; padding: 0 10px; pointer-events: none;">
            <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
                <span style="font-size: 8px; color: #9ca3af; text-transform: uppercase; font-weight: 900; letter-spacing: 0.1em; margin-bottom: 3px;">Entrada</span>
                <span style="font-size: 12px; font-weight: 900; color: white; line-height: 1; display: inline-flex; align-items: center; gap: 3px; white-space: nowrap;">
                    <span style="font-size: 10px;">{{ isDemoMode ? '🎮' : '🪙' }}</span>
                    <span>{{ formatCurrency(room.entryFee) }}</span>
                </span>
            </div>
            ...
            <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
                <span style="font-size: 8px; color: #9ca3af; text-transform: uppercase; font-weight: 900; letter-spacing: 0.1em; margin-bottom: 3px;">Mínimo</span>
                <span style="font-size: 14px; font-weight: 900; color: #34d399; line-height: 1; text-shadow: 0 1px 2px rgba(0,0,0,0.5); display: inline-flex; align-items: center; gap: 3px; white-space: nowrap;">
                    <span style="font-size: 10px;">{{ isDemoMode ? '🎮' : '🪙' }}</span>
