...
const fazerCadastro = async () => {
  ...
  try {
    ...
    // 🔥 CORREÇÃO DA MÁGICA: Se ele usou código na hora do cadastro, vai pra PENDENTE, e NÃO has_agent
    if (form.referralCode) {
      localStorage.setItem('magic_pending_agent', 'true'); 
    } else {
      localStorage.removeItem('magic_pending_agent');
    }
    
    localStorage.removeItem('magic_has_agent'); // Garante que não é VIP
    localStorage.setItem('magic_lobby_mode', 'TREINO'); 
...
.messages-area { text-align: center; min-height: 12px; margin-top: 0px; } 
...
.auth-footer { text-align: center; margin-top: 10px; color: #888; font-size: 12px; } 
