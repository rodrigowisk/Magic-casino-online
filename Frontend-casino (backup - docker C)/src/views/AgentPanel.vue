    ...
    <main class="content-area">
      ...
      <div v-if="!isLoading && !isAgent" class="become-agent-wrapper">
        <div class="become-agent-box">
          ...
          <div v-if="hasAgentLocal">
            <div class="icon-wrapper-large">🛡️</div>
            <h2>Conta Vinculada!</h2>
            <p>Sua conta já está vinculada a um Agente VIP. Aproveite as mesas exclusivas no Lobby Principal.</p>
            <button class="btn-primary" @click="voltar" style="margin-top: 15px;">IR PARA O LOBBY</button>
          </div>

          <div v-else-if="isPendingLocal">
            <div class="icon-wrapper-large">⏳</div>
            <h2 style="color: #f59e0b;">Aprovação Pendente</h2>
            <p>Sua solicitação de vínculo foi enviada com sucesso! Aguarde o agente aceitar o seu convite para liberar o acesso VIP.</p>
            <button class="btn-primary" @click="voltar" style="margin-top: 15px;">VOLTAR AO LOBBY</button>
          </div>

          <div v-else>
            <div class="icon-wrapper-large">🤝</div>
            <h2>Vincular Afiliado</h2>
            <p>Insira o Código de Convite do seu Afiliado para solicitar vínculo à sua conta.</p>
            
            <form @submit.prevent="vincularAfiliado" class="agent-form">
              <div class="input-group">
                <label>Código de Convite</label>
                <input 
                  type="text" 
                  v-model="inviteCode" 
                  @input="inviteCode = inviteCode.replace(/[^a-zA-Z0-9]/g, '').toUpperCase()"
                  placeholder="Ex: VIP777" 
                  required 
                  maxlength="20"
                />
              </div>
              ...
              <button type="submit" class="btn-primary" :disabled="isProcessing">
                {{ isProcessing ? 'SOLICITANDO...' : 'SOLICITAR VÍNCULO' }}
              </button>
            </form>
          </div>
      ...
      <div v-else-if="!isLoading && isAgent" class="dashboard-container">
        ...
        <div class="page-header">
          ...
          <div class="header-titles">
            ...
            <p>Gerencie suas indicações e acompanhe suas recompensas.</p>
          </div>
        </div>
        ...
        <div class="dashboard-grid">
          ...
          <div class="left-column">
            ...
            <div class="dash-card">
              ...
              <h3 class="card-value text-neon">🪙 {{ formatCurrency(agentData.agentBalance) }}</h3>
            ...
            <div class="dash-card">
              ...
              <div class="config-row mt-2">
                <div>
                  <span class="card-label">Ciclo de Fechamento</span>
            ...
            <div class="dash-card bg-green-dark relative overflow-hidden">
              ...
              <h3 class="card-value text-white">🪙 {{ formatCurrency(comissaoDisponivel) }}</h3>
              <p class="card-hint">As recompensas são distribuídas pelo administrador.</p>
              ...
              <div class="table-responsive">
                        ...
                        <td class="text-left">
                          <div class="player-cell">
                            ...
                            <div style="display: flex; flex-direction: column; justify-content: center;">
                              ...
                              <span class="player-id-small" v-if="player.id || player.Id">ID: {{ formatShortId(player.id || player.Id) }}</span>
                            </div>
                          </div>
                        </td>
                ...
                <div v-else-if="activeTab === 'history'">
                  <table class="players-table" v-if="transactionsList.length > 0">
                    ...
                    <tbody>
                      <tr v-for="tx in historicoFormatado" :key="tx.id">
                        ...
                        <td class="text-left text-white font-bold">
                          <div class="player-cell" style="display: flex; align-items: center; gap: 10px;">
                            ...
                            <div style="display: flex; flex-direction: column; gap: 2px;">
                              <span class="text-white">{{ tx.titulo }}</span>
                        ...
                        <td class="text-right font-bold" :class="tx.isPositive ? 'text-neon' : 'text-red-400'">
      ...
      <div class="modal-box">
        ...
        <div v-if="transactionSuccess" class="success-screen">
          ...
          <p class="success-details">
            <span v-if="transactionType === 'send'">Você enviou <strong>🪙 {{ formatCurrency(Number(transferAmount)) }}</strong> para o jogador</span>
            <span v-else>Você retirou <strong>🪙 {{ formatCurrency(Number(transferAmount)) }}</strong> do jogador</span>
        ...
        <div v-else>
          ...
          <div class="player-modal-header">
            ...
            <div class="player-modal-info">
              ...
              <span class="player-modal-balance">Saldo atual: <strong>🪙 {{ formatCurrency(selectedPlayerObj?.balance || 0) }}</strong></span>
            </div>
          </div>
          ...
          <form @submit.prevent="confirmarTransferencia" class="modal-form">
            <div class="input-group">
              <div class="input-header-flex">
                ...
                <span v-if="transactionType === 'send'" class="modal-desc text-right">Seu saldo: 🪙 {{ formatCurrency(agentData.agentBalance) }}</span>
                <span v-else class="modal-desc text-right text-red-400">Limite: 🪙 {{ formatCurrency(selectedPlayerObj?.balance || 0) }}</span>
              </div>
              <input 
                type="number" 
...
// 🔥 NOVO: Armazena o valor total da comissão ganha
const comissaoDisponivel = ref(0);
...
const historicoFormatado = computed(() => {
  ...
  return transactionsList.value.map((tx, index) => {
    const isEntrada = tx.amount > 0;
    const absAmount = Math.abs(tx.amount);
    const counterparty = tx.description || 'Sistema';
    const me = 'VOCÊ (Agente)';

    let titulo = 'Transferência';
    let rota = '';

    if (tx.operation === 'Transfer' || !tx.operation) {
        titulo = 'Transferência P2P';
        rota = isEntrada ? `${counterparty} ➔ ${me}` : `${me} ➔ ${counterparty}`;
    } else if (tx.operation === 'BuyIn') {
        titulo = 'Depósito na Mesa (Buy-in)';
        rota = `${me} ➔ Mesa de Jogo`;
    } else if (tx.operation === 'CashOut') {
        titulo = 'Retorno da Mesa (Cash-out)';
        rota = `Mesa de Jogo ➔ ${me}`;
    } else {
        titulo = isEntrada ? 'Recebimento de Fichas' : 'Retirada de Fichas';
        rota = isEntrada ? `Clube ➔ ${me}` : `${me} ➔ Clube`;
    }
...
// 🔥 FUNÇÃO DE CARREGAMENTO ATUALIZADA COM O CÁLCULO DO RAKE 🔥
const carregarPainel = async () => {
  ...
  try {
    ...
    if (data) {
      ...
      pendingList.value = data.pendingPlayers || data.PendingPlayers || [];

      // Inclui o próprio agente na lista para ganhar comissão sobre ele mesmo
      ...
      // 👉 AQUI ACONTECE A MÁGICA DE BUSCAR O RAKE NA API DE JOGOS
      let allPlayersRake: any[] = [];
      try {
         const baseUrlGame = import.meta.env.VITE_GAME_API_URL || 'http://localhost:5002';
         const token = localStorage.getItem('magic_token');
         const rakeResp = await fetch(`${baseUrlGame}/api/Table/report/all-players-rake`, {
            headers: { 'Authorization': `Bearer ${token}` }
         });
         
         if (rakeResp.ok) {
            allPlayersRake = await rakeResp.json();
         }
      } catch(e) {
         console.error("Erro ao buscar rake dos jogadores", e);
      }

      // Calcula as comissões individuais e soma tudo
      let totalCommissionEarned = 0;
      const commRate = agentData.value.commissionRate / 100; // Ex: 10% vira 0.1

      pList.forEach((p: any) => {
         const targetId = String(p.id || p.Id).toLowerCase();
         const playerRakeData = allPlayersRake.find(r => r.playerId.toLowerCase() === targetId);
         
         const rakeGerado = playerRakeData ? playerRakeData.rakePago : 0;
         const comissaoDesteJogador = rakeGerado * commRate;
         
         p.commission = comissaoDesteJogador;
         totalCommissionEarned += comissaoDesteJogador;
      });

      // Atualiza as variáveis reativas do Vue para exibir na tela
      playersList.value = pList;
      comissaoDisponivel.value = totalCommissionEarned;
...
const vincularAfiliado = async () => {
  ...
  try {
    ...
    const response = await fetch(`${IDENTITY_API_URL}/api/user/bind-agent`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
        },
        body: JSON.stringify({ referralCode: inviteCode.value })
    });

    const data = await response.json().catch(() => ({}));

    if (!response.ok) {
        if (data.message && data.message.includes('já enviou uma solicitação')) {
            localStorage.setItem('magic_pending_agent', 'true');
            isPendingLocal.value = true;
            return; 
        }
        throw new Error(data.message || "Erro ao solicitar vínculo. Verifique o código fornecido.");
    }
...
const aceitarJogador = async (playerId: string) => {
  ...
  try {
    ...
    const response = await fetch(`${IDENTITY_API_URL}/api/agent/accept-player/${playerId}`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
    });
    ...
    if (response.ok) {
        ...
        showNotification("Jogador aprovado e vinculado com sucesso!", "success"); 
    } else {
        const err = await response.json().catch(() => ({}));
        showNotification(err.message || "Erro ao aprovar jogador.", "error");
    }
  } catch (error) {
    console.error(error);
...
const rejeitarJogador = async (playerId: string) => {
  ...
  try {
    ...
    const response = await fetch(`${IDENTITY_API_URL}/api/agent/reject-player/${playerId}`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
    });
    ...
    if (response.ok) {
        ...
        showNotification("Erro ao rejeitar a solicitação.", "error");
    }
  } catch (error) {
    console.error(error);
  } finally {
...
onMounted(async () => {
  ...
  hasAgentLocal.value = localStorage.getItem('magic_has_agent') === 'true';
  isPendingLocal.value = localStorage.getItem('magic_pending_agent') === 'true';
...
.text-neon { color: #39FF14; font-weight: 900; }

.badge-voce { color: #f59e0b; font-size: 10px; font-weight: 900; margin-left: 6px; text-transform: uppercase; }
...
.toast-notification.success { background: linear-gradient(135deg, #10b981, #047857); border: 1px solid #34d399; }
.toast-notification.error { background: linear-gradient(135deg, #ef4444, #b91c1c); border: 1px solid #f87171; }
...
.main-layout { width: 100%; height: 100vh; height: 100dvh; display: flex; flex-direction: column; background-color: #0b0f19; color: white; font-family: 'Montserrat', sans-serif; overflow: hidden; }

.content-area { flex: 1; padding: 20px 40px; overflow-y: auto; overflow-x: hidden; box-sizing: border-box; padding-bottom: 90px; display: flex; flex-direction: column; }

@media (max-width: 768px) { .content-area { padding: 15px 5px; padding-bottom: 90px; } }

.dashboard-container { max-width: 1100px; margin: 0 auto; width: 100%; flex: 1; display: flex; flex-direction: column; }

.page-header { display: flex; align-items: center; gap: 15px; margin-bottom: 25px; padding: 0 10px; }

.btn-back { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: white; font-size: 18px; width: 35px; height: 35px; border-radius: 50%; cursor: pointer; display: flex; justify-content: center; align-items: center; transition: all 0.2s; }
.btn-back:hover { background: rgba(255,255,255,0.1); }

.header-titles { display: flex; flex-direction: column; }
.title-row { display: flex; align-items: center; gap: 10px; }
.title-row h2 { margin: 0; font-size: 20px; font-weight: 900; }
.badge-afiliado { background: rgba(245, 158, 11, 0.2); color: #f59e0b; padding: 2px 8px; border-radius: 4px; font-size: 9px; font-weight: 700; border: 1px solid rgba(245, 158, 11, 0.4); }
.header-titles p { margin: 2px 0 0 0; font-size: 12px; color: #9ca3af; }

.dashboard-grid { display: grid; grid-template-columns: 320px 1fr; gap: 20px; align-items: stretch; padding: 0 10px; flex: 1; }
...
.dash-card { background: #111827; border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 20px; box-shadow: 0 4px 6px rgba(0,0,0,0.3); }
...
.table-card { padding: 15px 0; flex: 1; display: flex; flex-direction: column; }
...
.nav-link { display: flex; align-items: center; gap: 8px; text-decoration: none; opacity: 0.5; transition: all 0.2s; padding-bottom: 4px; border-bottom: 2px solid transparent; }
.nav-link:hover { opacity: 0.8; }
.nav-link.active { opacity: 1; color: white; border-bottom: 2px solid #f59e0b; }

.visible-mobile { display: none; }
.hidden-mobile { display: inline; }

.table-responsive { overflow-x: auto; overflow-y: auto; width: 100%; padding: 0 10px; box-sizing: border-box; flex: 1; }
...
/* 🔥 ESTILOS PARA O ID NA LISTA DE JOGADORES 🔥 */
...
.avatar-image-container { width: 36px; height: 36px; border-radius: 50%; overflow: hidden; background-color: #374151; display: flex; justify-content: center; align-items: center; flex-shrink: 0; }
.avatar-img { width: 100%; height: 100%; object-fit: cover; }

.empty-players { padding: 30px 10px; text-align: center; color: #6b7280; font-size: 12px; font-style: italic; }

.btn-action-enviar { background: rgba(59, 130, 246, 0.1); border: 1px solid rgba(59, 130, 246, 0.3); color: #60a5fa; padding: 6px 12px; border-radius: 6px; font-size: 10px; font-weight: 700; cursor: pointer; display: inline-flex; align-items: center; gap: 4px; transition: 0.2s; }
...
/* ESTILOS PARA O VÍNCULO VIP */
.become-agent-wrapper { display: flex; justify-content: center; align-items: center; height: 70vh; flex: 1;}
.become-agent-box { background: #111827; border: 1px solid #38bdf8; border-radius: 16px; padding: 30px; max-width: 400px; text-align: center; box-shadow: 0 10px 30px rgba(0,0,0,0.5); margin: 0 15px;}
.icon-wrapper-large { font-size: 40px; margin-bottom: 10px; }
.become-agent-box h2 { color: #38bdf8; margin: 0 0 10px 0; font-size: 22px; font-weight: 900;}
...
.agent-form input:focus { border-color: #38bdf8; }

.btn-primary { background: #38bdf8; color: black; border: none; padding: 12px; width: 100%; border-radius: 8px; font-weight: 900; font-size: 14px; cursor: pointer; transition: 0.2s; box-shadow: 0 4px 10px rgba(56, 189, 248, 0.3);}
...
.avatar-large-image-container { width: 50px; height: 50px; border-radius: 50%; overflow: hidden; background: linear-gradient(135deg, #10b981, #047857); display: flex; justify-content: center; align-items: center; border: 2px solid #064e3b; }
