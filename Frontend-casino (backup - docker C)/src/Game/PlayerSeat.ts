...
export class PlayerSeat {
    ...
    // 🔥 Adicionado o filtro de cor para o efeito de escurecimento
    ...
    constructor(
        ...
        // Pré-configura o filtro para remover o brilho
    ...
    public updateTimer(progress: number, secLeft: number) {
        ...
        // 🔥 SOLUÇÃO DO RELÓGIO: Limite agora é sutil (0.002). Permite fluidez de 60 FPS reais sem travar.
    ...
    // 🔥 CORREÇÃO AQUI: Aplica o filtro em vez de mexer na transparência
    public darken() {
        if (!this.container || this.container.destroyed) return;
        if (this.seatedContainer && !this.seatedContainer.destroyed) { 
            this.seatedContainer.filters = [this.colorFilter]; // Tira o brilho
            this.seatedContainer.alpha = 1.0; // Garante que a imagem fique sólida, sem transparência
        }
    }

    // 🔥 CORREÇÃO AQUI: Remove o filtro e reseta para o estado normal
    public resetFilter() {
        if (!this.container || this.container.destroyed) return;
        if (this.seatedContainer && !this.seatedContainer.destroyed) {
            this.seatedContainer.filters = []; // Tira o filtro de cor
            if (!this.sitAnimTick && !this.standAnimTick) { 
                this.seatedContainer.alpha = 1.0; // Mantém a opacidade
            }
        }
    }
