    ...
    public async playBetAnimation(
        ...
        try {
            ...
            await new Promise<void>((resolve) => {
                ...
                const existingCenter = this.engine.dealtCardsUI.filter(c => Math.abs(c.x - 215) < 5 && Math.abs(c.y - 420) < 5);
            ...
            if (isWin) {
                ...
                const centerCards = this.engine.dealtCardsUI.filter(c => Math.abs(c.x - 215) < 5 && Math.abs(c.y - 420) < 5 && !c.destroyed);
