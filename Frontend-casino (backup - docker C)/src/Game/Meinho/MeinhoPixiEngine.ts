...
export class MeinhoPixiEngine {
    ...
    public async init(canvasContainer: HTMLElement, width: number, height: number, defaultAvatarImg: string, deckImg: string, singleChipImg: string, tableImg: string, potChipsImg: string) {
        ...
        // 🔥 CORREÇÃO DA ORDEM DAS CAMADAS:
        // A mainLayer é adicionada ANTES da particleLayer.
        // Assim, as partículas brilham POR CIMA do avatar e do relógio!
    ...
    public buildSeats(numSeats: number) {
        this.playerSeats.forEach(seat => {
            ...
            else if (seat.container && !seat.container.destroyed) {
    ...
    public setHeroCardsVisibility(visible: boolean) { this.isHeroCardsHidden = !visible; }
    ...
    public destroy() {
        ...
        this.playerSeats.forEach(seat => {
            if (seat && typeof seat.destroy === 'function') { seat.destroy(); } 
            else if (seat.container && !seat.container.destroyed) { seat.container.destroy({ children: true }); }
        });
    ...
    private updateFramePixi() {
        ...
        // 🔥 CORREÇÃO: Partículas na ponta do relógio aprimoradas para máximo brilho!
        if (progress > 0 && progress < 1 && Math.random() > 0.1) { 
            ...
            p.circle(0, 0, Math.random() * 3 + 2); // Tamanho da partícula maior
            
            // Mistura branco com a cor para ficar incandescente
            p.fill({ color: Math.random() > 0.7 ? 0xffffff : color, alpha: 1 });
            p.blendMode = 'add'; // 💥 EFEITO MÁGICO: Transforma a partícula em "luz"
            ...
            this.activeFireParticles.push({
                ...
                vy: (Math.random() - 0.5) * 2 - 1 // Gravidade puxando para cima como fumaça
            });
        }
    }
