...
// 🔥 LÓGICA DE MOSTRAR O BOTÃO BLINDADA E À PROVA DE FALHAS
const verificarAberturaFilar = () => {
    ...
    if (cards && cards.length === 2 && hero && hero.status === 'playing' && peekModeEnabled.value && !hasPeekedCurrentHand.value && !hasManuallyRevealedCards.value) {
        ...
        // Loop rigoroso de verificação (Polling)
        const checkEngineIdle = () => {
            // Verifica o estado da engine diretamente a cada 250ms
            if (isDealing.value || isAnimating.value || queuedAnimations.length > 0 || isDiscarding3D.value) {
                ...
                // Engine totalmente ociosa, adicionamos apenas uma pequena margem para a renderização do frame final
                peekTimeoutId = setTimeout(tryOpenPeeker, 300);
            }
        };

        // Dá 400ms inicialmente para garantir que a renderização começou antes de verificar
    ...
    if (isMyTurnToSit.value) {
        const occupiedLogicalSeats = gameState.players
            ...
            .map(p => {
                ...
                return (visualIndex + myLogicalSeatOffset.value) % gameState.maxPlayers;
            });
...
function applyState(serverState: any, isSnap = false) {
    try {
        ...
        normState.players.forEach((p: any) => {
            if (p.isSeated && p.seat >= 0 && p.seat < gameState.maxPlayers) {
                ...
                if (player) {
                    ...
                    player.isHero = (currentUserId && p.userId === currentUserId);
        ...
        if (heroPlayer && heroPlayer.isSeated && (heroPlayer.chips + heroPlayer.pendingRebuy) <= 0 && heroPlayer.status !== 'playing') {
            if (normState.phase !== 'resolving') {
                fetchUserBalance().then(() => { if(!isManualRebuy.value) showRebuyModal.value = true; });
            }
        ...
        let newVisualTurn = -1;
        if (normState.currentTurnSeat !== -1) newVisualTurn = (normState.currentTurnSeat - myLogicalSeatOffset.value + gameState.maxPlayers) % gameState.maxPlayers;
        ...
        if (gameState.phase === 'betting' && newVisualTurn !== -1) {
            if (newVisualTurn !== gameState.currentTurn || oldPhase !== 'betting' || forceSnap) {
                ...
                if (engine) {
                    engine.startTimer(newVisualTurn, gameState.turnTimeLeft > 0 ? gameState.turnTimeLeft : 20);
                }
            }
        } else {
...
function syncTable(serverState: any) {
    const meInServer = (serverState.players || []).find((p:any) => p.userId === currentUserId && p.isSeated);
...
const handleVisibilityChange = async () => {
    if (document.visibilityState === 'hidden') {
        ...
        let isHeroNext = false;
        if (pendingState && pendingState.currentTurnSeat !== undefined && pendingState.currentTurnSeat !== -1) {
            const hero = gameState.players.find(p => p.isHero && p.isSeated);
            if (hero && hero.seat === pendingState.currentTurnSeat) {
                isHeroNext = true;
            }
        } else if (gameState.currentTurn !== -1 && gameState.players[gameState.currentTurn]?.isHero) {
            isHeroNext = true;
        }

        if (timeAway > 3000 || isHeroNext) {
            if (pendingState) {
                const stateToApply = pendingState;
                pendingState = null;
                forceNextSyncAsSnap = true; 
