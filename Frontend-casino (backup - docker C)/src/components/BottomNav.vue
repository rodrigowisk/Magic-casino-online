<template>
  <nav class="bottom-nav">
    ...
    <router-link v-if="!isAdmin" to="/agente" class="nav-item" active-class="active">
      ...
      <span>{{ isAgent ? 'Agente' : 'Afiliado' }}</span>
    </router-link>
...
onMounted(() => {
  // Verifica se o usuário tem as permissões corretas (à prova de case sensitivity)
  ...
  if (rolesStr) {
    try {
      ...
      isAdmin.value = rolesLower.includes('owner') || rolesLower.includes('admin');
      isAgent.value = rolesLower.includes('agent') || rolesLower.includes('agente');
    } catch (e) {
      // Fallback de segurança se não for um JSON válido
