      ...
      <div class="menu-body">
        ...
        <div class="stats-list">
          <div v-for="p in activePlayers" :key="p.userId" class="stats-grid stats-row">
            ...
            <div class="col-buyin text-muted">{{ (p.totalBuyIn || 0).toFixed(2).replace('.', ',') }}</div>
...
// 🔥 Ícones de moedas removidos daqui 🔥
const formatProfit = (player: any) => {
  const profit = (player.chips + (player.totalCashOut || 0)) - (player.totalBuyIn || 0);
  if (profit > 0) return `+ ${profit.toFixed(2).replace('.', ',')}`;
  if (profit < 0) return `- ${Math.abs(profit).toFixed(2).replace('.', ',')}`;
  return '0,00';
};
</script>
