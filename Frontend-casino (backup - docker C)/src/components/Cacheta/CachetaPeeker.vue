...
// Importa o verso da carta (Ajuste o caminho conforme a estrutura do seu projeto)
import deckImg from '../../assets/imagens/deck.png';

// 👇 Modificado para receber apenas UMA carta
...
// 👇 Modificado para gerenciar apenas UMA carta
...
function initThreeJS() {
  ...
  // 👇 Passando a prop "card" diretamente, já que agora é um único objeto/string
  const texFront1 = createFrontTexture(props.card);
  ...
  // 👇 Centralizando a carta e removendo a rotação em Z para ela ficar reta
  card1.position.set(0, 0, 0);
  card1.rotation.z = 0; 
  scene.add(card1);
}

function applyBend(amount: number) {
  if (!geomCard1Front) return;
  ...
  // 👇 Deformando apenas a carta única
...
function animate() {
  ...
  if (isDiscarding.value && card1) {
    // 3D Engine Animation para descartar/guardar
