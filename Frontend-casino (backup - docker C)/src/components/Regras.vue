<template>
  <div class="legal-container">
    ...
    <div class="tabs">
      <button 
        v-if="activeGames.meinho" 
        :class="{ active: activeTab === 'meinho' }" 
        @click="activeTab = 'meinho'">
        Meinho
      </button>
      
      <button 
        v-if="activeGames.cacheta" 
        :class="{ active: activeTab === 'cacheta' }" 
        @click="activeTab = 'cacheta'">
        Cacheta
      </button>
    </div>
    ...
    <div class="content-box custom-scrollbar" ref="contentBox">
      ...
      <div v-if="activeGames.meinho && activeTab === 'meinho'" class="document-text fade-in">
      ...
      <div v-if="activeGames.cacheta && activeTab === 'cacheta'" class="document-text fade-in">
...
// Lógica inteligente para definir a aba inicial baseada na URL e na disponibilidade do jogo
let initialTab = (route.params.jogo as string);

// Se não houver parâmetro, ou se o jogo pedido estiver desativado, pegamos o primeiro disponível
if (!initialTab || (initialTab === 'cacheta' && !activeGames.value.cacheta) || (initialTab === 'meinho' && !activeGames.value.meinho)) {
  initialTab = activeGames.value.meinho ? 'meinho' : (activeGames.value.cacheta ? 'cacheta' : '');
}

const activeTab = ref(initialTab);

onMounted(() => {
  // Proteção: Se tudo estiver desligado, volta pro lobby
  if (!activeGames.value.meinho && !activeGames.value.cacheta) {
    router.push('/lobby');
    return;
  }
